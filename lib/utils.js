...
let ipBindCallback = null;

/**
 * Set callback that returns the IP address to bind an HTTPS request to
 *
 * @param {function} callback
 * @returns {void}
 */
exports.setIpBindCallback = (callback) => {
  ipBindCallback = callback;
}

/**
 * Get the IP address to bind an HTTPS request to
 *
 * @returns {Agent}
 */
exports.getIpBind = () => {
  if (!ipBindCallback) return undefined;
  const ip = ipBindCallback();
  console.log(ip);
  return ip;
}
...
exports.exposedMiniget = (url, options = {}, requestOptionsOverwrite) => {
  const usedOptions = requestOptionsOverwrite || options.requestOptions;
  const req = miniget(url, {
    agent: exports.getIpBind(),
    ...usedOptions
  });
